...
void solve() {
  ...
  map<char, map<char, int>> mpc;
  for (int i = 0; i < n; i++) {
    //traversing with auto is slower as it copies whole map object
    for (char j = 'a'; j <= 'z'; j++) {
      mpc[str[i]][j] += mp[j][i + 1];
      mx = max(mx, mpc[str[i]][j]);
    }
  }
  cout << mx;
}
